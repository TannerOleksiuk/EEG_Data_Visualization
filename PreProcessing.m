...
% This script requires the function getEvent.m, plotEvent.m as well as the signal
...
%Prompt user to enter threshold, (originally had 200 here but that was based off assumptions, best to allow user to decide
%since this is application specific)
threshold = input("Please enter a threshold for artifact rejection variance (recommended 200 if unsure): ");
...
for i = 1:numRelEvents
    for j = 1:eventCount(find(types==events(i+(numEvents-numRelEvents)).type))
        event = artifactRejected.(strcat("T",int2str(events(i+(numEvents-numRelEvents)).type))).(strcat("eventNum",int2str(j)));
        for n = 1:numCH
...
disp("Saving baseline corrected data....");
...
plotEvent(epochedDat,types,eventCount); %Plots Baseline Corrected Events (Not artifact rejected)
